...
protected:
            ...
            for (size_t eventIdx = 0; eventIdx < m_midi[trackIdx].size(); ++eventIdx) {
                ...
                switch (event.type()) {
                    ...
                    case MidiEventType::note_on: {
                        m_noteMap[trackIdx].emplace_back(time, MidiTimeMode::tick, trackIdx, channel, event[1],
                            ...
                            case MidiMetaType::time_signature: {
                                ...
                                if (m_bbMap[metaTrack].size()) {
                                    ...
                                    deltaBeat = (time - m_bbMap[metaTrack].back().time) *
                                                m_bbMap[metaTrack].back().denominator / 4.0 / m_head.tpqn();
                                    newBeat = m_bbMap[metaTrack].back().beatNode + deltaBeat;
                                    newBar = m_bbMap[metaTrack].back().barNode +
                                             deltaBeat / m_bbMap[metaTrack].back().numerator;
                                }
                                else {
                                    newBeat = time * 1.0 / m_head.tpqn();
                                    newBar = newBeat / 4;
                                }
                                m_bbMap[metaTrack].emplace_back(time, MidiTimeMode::tick, trackIdx, newBar, newBeat,
                                                                event[3], 1 << event[4]);
                                break;
                            }
                            default: {
    ...
    MidiErrorCode change_timeMode_unsorted(MidiEventMap<_MidiEvent>& _map, _Fun&& _fun, _GetTime&& _getTime) const {
        for (size_t trackIdx = 0; trackIdx < _map.size(); ++trackIdx) {
            ...
            for (size_t eventIdx = 0; eventIdx < _map[trackIdx].size(); ++eventIdx) {
                _MidiEvent& event = _map[trackIdx][eventIdx];
                size_t tempoIdx = _find_tempoIdx_between(event.time, m_tempoMap[metaTrack], 0,
                                                         m_tempoMap[metaTrack].size(), _getTime);
    ...
    MidiErrorCode change_timeMode_tick2micro_sorted(MidiEventMap<_MidiEvent>& _map) const {
        ...
        return change_timeMode_sorted(_map, __tick2micro_fun<_MidiEvent>(m_head.tpqn()), __get_tempo_timeNode_fun());
    }
    ...
    MidiErrorCode change_timeMode_micro2tick_sorted(MidiEventMap<_MidiEvent>& _map) const {
        ...
        return change_timeMode_sorted(_map, __micro2tick_fun<_MidiEvent>(m_head.tpqn()), __get_tempo_timeNode_fun());
    }
    ...
    MidiErrorCode change_timeMode_tick2micro_unsorted(MidiEventMap<_MidiEvent>& _map) const {
        ...
        return change_timeMode_unsorted(_map, __tick2micro_fun<_MidiEvent>(m_head.tpqn()), __get_tempo_timeNode_fun());
    }
    ...
    MidiErrorCode change_timeMode_micro2tick_unsorted(MidiEventMap<_MidiEvent>& _map) const {
        ...
        return change_timeMode_unsorted(_map, __micro2tick_fun<_MidiEvent>(m_head.tpqn()), __get_tempo_timeNode_fun());
    }
