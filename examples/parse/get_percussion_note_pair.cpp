...
int main() {
    ...
    for (const NotePair& notePair : notePairList) {
        fprintf(file, "%llu\t%llu\t0x%2.2X\t0x%2.2X\t0x%2.2X\t0x%2.2X\t%.3f\t%.3f\t%.3f\t%.3f\n", notePair.time,
                notePair.duration, notePair.channel, notePair.pitch, notePair.instrument, notePair.velocity,
                notePair.bar, notePair.bar + notePair.bar_diff, notePair.beat, notePair.beat + notePair.beat_diff);
    }
    fclose(file);
